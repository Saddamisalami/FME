...
void play_furelise(int buzzer)
{
  ...
  const int notes = sizeof(melody_furelise) / sizeof(melody_furelise[0]) / 2;

  // this calculates the duration of a whole note in ms
  const int wholenote = (60000 * 4) / tempo_furelise;
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2)
  {
    ...
    divider = melody_furelise[thisNote + 1];
    if (divider > 0)
    {
...
#if defined(ESP32)
    ledcWriteTone(soundChannel, melody_furelise[thisNote]);
#else
    tone(buzzer, melody_furelise[thisNote], noteDuration * 0.9);
#endif
