...
void play_asabranca(int buzzer)
{

  // change this to make the song slower or faster
  const int tempo_asabranca = 120;

  // notes of the moledy followed by the duration.
  // a 4 means a quarter note, 8 an eighteenth , 16 sixteenth, so on
  // !!negative numbers are used to represent dotted notes,
  // so -4 means a dotted quarter note, that is, a quarter plus an eighteenth!!
  const int melody_asabranca[] = {
  ...
  const int notes = sizeof(melody_asabranca) / sizeof(melody_asabranca[0]) / 2;

  // this calculates the duration of a whole note in ms
  const int wholenote = (60000 * 4) / tempo_asabranca;
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2)
  {
    ...
    divider = melody_asabranca[thisNote + 1];
    if (divider > 0)
    {
